...
reported <- reported_raw %>% 
  ...
  mutate(status = fct_relevel(status, c("Active", "Recovered", "Deceased")))
```
...
outbreak <- outbreak_raw %>% 
  ...
  mutate(outbreak_or_sporadic = fct_relevel(outbreak_or_sporadic, 
                                    c("Sporatic", "Outbreak associated")))
```
...
income <- nbhood_profile %>% 
  ...
  mutate(neighbourhood_name = str_replace_all(neighbourhood_name, "St.\\s", "St.")) %>%
...
ggplot(data = nbhoods_final) +
  ...
  labs(title = "COVID−19 cases and low-income status by neighbourhood in Toronto, Canada",
       caption = str_c("Created by: Andy Vu for STA303/1002, U of T\n",
                       ...
                       date_daily[1,1])) +
  scale_fill_brewer(name= paste("% of 18 to 64 year−olds in\nlow income families and\n", 
                                "COVID−19 case rates", sep = ""), palette = "Set1")
```
